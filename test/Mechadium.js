...
const deployWithoutLGE = deployments.createFixture(async ({ deployments, ethers }, options) => {
  ...
  const pad = await ethers.getContract("Mechadium");
...
describe("Mechadium", () => {
  describe("Deployment", () => {
    it("name should be Mechadium", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.name()).to.be.equal("Mechadium");
    });

    it("symbol should be MECHAX", async () => {
      const { pad } = await deployWithLGE();
      expect(await pad.symbol()).to.be.equal("MECHAX");
    });
    ...
    it("deployer should have the total initial supply", async () => {
      const { pad, deployer } = await deployWithLGE();
      expect(await pad.balanceOf(deployer.address)).to.be.equal(tokens("10000000000"));
    });
  });
